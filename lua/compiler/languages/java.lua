...
function M.action(selected_option)
  ...
  local output_dir = utils.os_path("../out/production/" .. vim.fn.fnamemodify(vim.fn.expand('%:p:h:h'), ':t')) -- working_directory/bin/
  ...
  elseif selected_option == "option3" then
    local task = overseer.new_task({
      ...
      strategy = {
        ...
        tasks = { {
          "shell",
          name = "- Run program (class) → " .. output .. ".class",
  ...
  elseif selected_option == "option7" then
    local task = overseer.new_task({
      ...
      strategy = {
        ...
        tasks = { {
          "shell",
          name = "- Run program (jar) → " .. output .. ".jar",
    ...
    if solution_file then
      ...
      if solution_executables then
        for entry, executable in pairs(solution_executables) do
          task = {
            "shell",
            name = "- Run program (jar) → " .. executable,
